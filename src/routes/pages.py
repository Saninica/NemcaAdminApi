...
@router.post("/", response_model=PageRead,  dependencies=[Depends(get_current_user)])
async def create_page(page: PageCreate, db: AsyncSession = Depends(get_db)):
    created_page = await crud_page.create(db, obj_in=page)
    return created_page


@router.get("/{page_id}", response_model=PageRead, dependencies=[Depends(get_current_user)])
...
@router.get("/", response_model=List[PageRead], dependencies=[Depends(get_current_user)])
...
@router.put("/{page_id}", response_model=PageRead, dependencies=[Depends(get_current_user)])
async def update_page(page_id: int, page: PageUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if not db_page:
        raise HTTPException(status_code=404, detail="Page not found")
    updated_page = await crud_page.update_with_contents(db, db_obj=db_page, obj_in=page)
    return updated_page
